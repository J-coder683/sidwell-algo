...
def fetch_financials(ticker: str) -> dict:
    is_india = ticker.endswith(".NS") or ticker.endswith(".BO")
    if is_india:
        return _fetch_financials_yfinance(ticker)  # TEMPORARY — replaced by screener.in in v0.6.3
    else:
        return _fetch_financials_fmp(ticker)

# TEMPORARY: yfinance path retained ONLY for Indian tickers (.NS / .BO) until
# v0.6.3 ships the screener.in scraper. Yahoo rate-limits US ticker scraping
# aggressively which broke the v0.6 demo on May 28. See BUILD_NOTES §21.
# DO NOT extend this function. DO NOT call it for US tickers.
def _fetch_financials_yfinance(ticker: str) -> dict:
    """
...
def _fetch_financials_fmp(ticker: str) -> dict:
    cache_key_fin = f"financials_fmp_{ticker}.json"
    cache_key_price = f"price_fmp_{ticker}.json"
    
    cached_fin = cache.get_json(cache_key_fin, TTL_FINANCIALS)
    cached_price = cache.get_json(cache_key_price, TTL_PRICES)
    
    if cached_fin is not None and cached_price is not None:
        logger.info(f"Loaded financials and price for {ticker} from FMP cache.")
        res = cached_fin.copy()
        res["current_price"] = cached_price["current_price"]
        return res
    ...
    def _fetch_endpoint(endpoint_url: str):
        ...
        if resp.status_code == 401:
            ...
            logger.warning(f"FMP returned 5xx for {url}, retrying in 3s...")
            time.sleep(3)
            resp = session.get(url, timeout=10)
            if not resp.ok:
                raise ValueError(f"FMP returned {resp.status_code}: {resp.text}")
        elif not resp.ok:
            raise ValueError(f"FMP returned {resp.status_code}: {resp.text}")
    ...
    try:
        ...
        inc_stmt_data = list(reversed(inc_stmt_data))
        ...
        for p in inc_stmt_data:
            years.append(p.get("date", "")[:10])
            revenue.append(float(p.get("revenue", 0.0) or 0.0))
            gross_profit.append(float(p.get("grossProfit", 0.0) or 0.0))
            ebit.append(float(p.get("operatingIncome", 0.0) or 0.0))
            interest_expense.append(float(p.get("interestExpense", 0.0) or 0.0))
            tax_provision.append(float(p.get("incomeTaxExpense", 0.0) or 0.0))
            pretax_income.append(float(p.get("incomeBeforeTax", 0.0) or 0.0))
        ...
        for p in bal_sheet_data:
            total_assets.append(float(p.get("totalAssets", 0.0) or 0.0))
            total_equity.append(float(p.get("totalStockholdersEquity", 0.0) or 0.0))
            c_val = p.get("cashAndShortTermInvestments")
            if c_val is None:
                c_val = p.get("cashAndCashEquivalents", 0.0)
            cash.append(float(c_val or 0.0))
            debt.append(float(p.get("totalDebt", 0.0) or 0.0))
            total_intangibles.append(float(p.get("intangibleAssets", 0.0) or 0.0))
        ...
        for p in cf_stmt_data:
            cx = float(p.get("capitalExpenditure", 0.0) or 0.0)
            # FMP returns negative for outflows, Sidwell expects positive
            capex.append(abs(cx))
            depreciation.append(float(p.get("depreciationAndAmortization", 0.0) or 0.0))
            working_capital_change.append(float(p.get("changeInWorkingCapital", 0.0) or 0.0))
        ...
        try:
            ...
            if float_data and len(float_data) > 0:
                ...
                if io is not None:
                    insider_ownership = float(io)
            else:
                logger.warning(f"FMP /shares-float unavailable for {ticker}, defaulting insider_ownership to 0.0")
        except ValueError as e:
            if "quota" in str(e).lower() or "invalid" in str(e).lower():
                raise
            logger.warning(f"FMP /shares-float error for {ticker}, defaulting insider_ownership to 0.0. {e}")
        ...
        try:
            ...
            if rec_data and len(rec_data) > 0:
                ...
                if scores:
                    recommendation_mean = sum(scores) / len(scores)
        except ValueError as e:
            if "quota" in str(e).lower() or "invalid" in str(e).lower():
                raise
        ...
        logger.error(f"Error fetching data from FMP for ticker {ticker}", exc_info=True)
        raise
