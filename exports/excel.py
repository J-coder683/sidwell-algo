...
def _build_assumptions(wb: Workbook, dcf_results: dict, financials: dict):
    ...
    data = [
        ...
        # Notes must begin with [SOURCE_TYPE] per AJP self-justifying contract.
        ("wacc",            "WACC (computed)",
         assumptions["wacc"],
         _pct_format(),
         "[ENGINE] Weighted Average Cost of Capital — avg of current and target structure WACC via Hamada unlever/relever"),
        ("terminal_growth", "Terminal Growth Rate",
         assumptions.get("terminal_growth_rate", 0.04),
         _pct_format(),
         "[AJP] " + assumptions.get("sector_terminal_source", "Sector-aware terminal growth; defaults to nominal GDP ~2-3%")),
        ("tax_rate",        "Effective Tax Rate",
         assumptions["tax_rate"],
         _pct_format(),
         "[FILING] 4-year historical average from company filings"),
        ("revenue_growth",  "Stage 1 Revenue Growth (CAGR)",
         assumptions.get("revenue_growth", assumptions.get("stage_1_growth", 0.10)),
         _pct_format(),
         "[FILING] 4-year historical sales CAGR from statements; capped at 5%–20%"),
        ("capex_sales",     "CapEx / Sales ratio",
         assumptions.get("capex_to_sales", 0.05),
         _pct_format(),
         "[FILING] 4-year average CapEx as % of revenue from cash flow statement"),
        ("nwc_sales",       "NWC Change / Sales ratio",
         assumptions.get("nwc_to_sales", 0.01),
         _pct_format(),
         "[FILING] 4-year average NWC change as % of revenue from balance sheet"),
        ("shares",          "Shares Outstanding",
         assumptions.get("shares_outstanding", 0),
         "#,##0",
         "[YAHOO] Shares outstanding from Yahoo Finance at time of export"),
        ("net_debt",        "Net Debt (Debt − Cash)",
         assumptions.get("latest_debt", 0) - assumptions.get("latest_cash", 0),
         _currency_format(is_india),
         "[FILING] Latest FY balance sheet: Total Debt less Cash & Equivalents"),
        ("rf_rate",         "Risk-Free Rate",
         assumptions.get("risk_free_rate", 0.065),
         _pct_format(),
         "[FRED] INDIRLTLT01STM — India 10Y Government Bond yield" if is_india else "[FRED] DGS10 — US 10Y Treasury yield"),
        ("erp",             "Total Equity Risk Premium",
         assumptions.get("total_erp", 0.07),
         _pct_format(),
         "[DAMODARAN] Mature market ERP (Damodaran Jan update) + country risk premium"),
        ("beta_levered",    "Levered Beta",
         assumptions.get("beta_levered", 1.0),
         "0.000",
         "[DAMODARAN] Industry median asset beta, re-levered using company D/E ratio (Hamada)"),
        ("cost_of_equity",  "Cost of Equity (Ke)",
         assumptions.get("cost_of_equity", 0.10),
         _pct_format(),
         "[ENGINE] CAPM: Rf + β × (mature ERP + CRP)"),
        ("cost_of_debt",    "Cost of Debt (Kd)",
         assumptions.get("cost_of_debt", 0.07),
         _pct_format(),
         "[ENGINE] " + assumptions.get("debt_source", "Synthetic rating: interest expense / average total debt")),
        ("equity_weight",   "Equity Weight (We)",
         assumptions.get("equity_weight", 0.90),
         _pct_format(),
         "[ENGINE] Market Cap / (Market Cap + Total Debt) at current structure"),
        ("debt_weight",     "Debt Weight (Wd)",
         assumptions.get("debt_weight", 0.10),
         _pct_format(),
         "[ENGINE] Total Debt / (Market Cap + Total Debt) at current structure"),
        ("ebitda_margin",   "Latest EBITDA Margin",
         assumptions.get("ebitda_margin", 0.18),
         _pct_format(),
         "[FILING] EBIT + D&A from latest full-year income statement"),
        ("stage1_growth",   "Stage 1 Growth Rate (for ref)",
         assumptions.get("stage_1_growth", assumptions.get("revenue_growth", 0.10)),
         _pct_format(),
         "[ENGINE] High-growth phase rate; same as revenue_growth for Stage 1 years 1-5"),
    ]
